...
import {  GitFileStatus, GitRepoSet, PullRequestConfig, PullRequestProvider, RepoDropdownOrder } from './types';
...
export function realpath(path: string, native: boolean = false) {
	return new Promise<string>((resolve) => {
		(native ? fs.realpath.native : fs.realpath)(path, (err, resolvedPath) => resolve(err !== null ? path : resolvedPath));
	});
}
 ...
 * @returns A promise resolving to the ErrorInfo of the executed command.
 */
export function openExternalUrl(url: string, type: string = 'External URL'){
 ...
 * @returns A promise resolving to the ErrorInfo of the executed command.
 */
export async function openFile(repo: string, filePath: string, hash: string | null = null, dataSource: DataSource | null = null, viewColumn: any | null = null) {
...
export function viewFileAtRevision(repo: string, hash: string, filePath: string) {
	...
	console.log('TODO-viewFileAtRevision');
}
 ...
 */
export async function findGit(extensionState: any) {
	const lastKnownPath = 'C:/Program Files/Git/bin/git.exe';
	...
	const configGitPaths = ['C:/Program Files/Git/bin/git.exe'];
 ...
 */
 export function openGitTerminal(cwd: string, gitPath: string, command: string | null, name: string) {
	...
	console.log('TODO - openGitTerminal');	
}
