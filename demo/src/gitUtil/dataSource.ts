...
import { GitExecutable, GitVersionRequirement, UNABLE_TO_FIND_GIT_MSG, UNCOMMITTED, abbrevCommit, constructIncompatibleGitVersionMessage, doesVersionMeetRequirement, getPathFromStr, openGitTerminal, pathWithTrailingSlash, realpath, resolveSpawnOutput } from './utils';
...
export class DataSource  {
	 ...
	 */
	getConfig(repo: string, remotes: ReadonlyArray<string>): Promise<GitRepoConfigData> {
		return Promise.all([
	...
	public getCommitFile(repo: string, commitHash: string, filePath: string) {
		return this._spawnGit(['show', commitHash + ':' + filePath], repo, stdout => {
			const encoding = DataSourceConfig.fileEncoding;
				...
				if (!err) {
					...
					for (let i = 0; i < lines.length; i++) {
						...
						if (inSubmoduleSection && (match = lines[i].match(pathProp)) !== null) {
							let root = await this.repoRoot(path.join(repo, getPathFromStr(match[1])));
	...
	public repoRoot(pathOfPotentialRepo: string) {
		return this.spawnGit(['rev-parse', '--show-toplevel'], pathOfPotentialRepo, (stdout) => path.normalize(stdout.trim())).then(async (pathReturnedByGit) => {
	...
	public addTag(repo: string, tagName: string, commitHash: string, type: TagType, message: string, force: boolean) {
		...
		if (type === TagType.Lightweight) {
			...
			args.push(DataSourceConfig.signTags ? '-s' : '-a', tagName, '-m', message);
		}
	...
	public pullBranch(repo: string, branchName: string, remote: string, createNewCommit: boolean, squash: boolean) {
		const args = ['pull', remote, branchName], config = DataSourceConfig;
		if (squash) {
			args.push('--squash');
		...
		return this.runGitCommand(args, repo).then((pullStatus) => {
			return pullStatus === null && squash
				? this.commitSquashIfStagedChangesExist(repo, remote + '/' + branchName, MergeActionOn.Branch, SquashMessageFormat.Default, config.signCommits)
				: pullStatus;
		});
	}
	...
	public merge(repo: string, obj: string, actionOn: MergeActionOn, createNewCommit: boolean, squash: boolean, noCommit: boolean) {
		const args = ['merge', obj], config = DataSourceConfig;
		if (squash) {
			args.push('--squash');
		} else if (createNewCommit) {
			args.push('--no-ff');
		...
		return this.runGitCommand(args, repo).then((mergeStatus) => {
			return mergeStatus === null && squash && !noCommit
				? this.commitSquashIfStagedChangesExist(repo, obj, actionOn, SquashMessageFormat.Default, config.signCommits)
				: mergeStatus;
		});
	}
	...
	public rebase(repo: string, obj: string, actionOn: RebaseActionOn, ignoreDate: boolean, interactive: boolean) {
		if (interactive) {
			return this.openGitTerminal(
				repo,
				'rebase --interactive ' + (DataSourceConfig.signCommits ? '-S ' : '') + (actionOn === RebaseActionOn.Branch ? obj.replace(/'/g, '"\'"') : obj),
			...
			if (DataSourceConfig.signCommits) {
				args.push('-S');
			}
			return this.runGitCommand(args, repo);
		}
	...
	public openExternalDirDiff(repo: string, fromHash: string, toHash: string, isGui: boolean) {
		return new Promise<ErrorInfo>((resolve) => {
			if (this.gitExecutable === null) {
				...
				if (isGui) {
					console.log('External diff tool is being opened (' + args[args.length - 1] + ')');
					this.runGitCommand(args, repo).then((errorInfo) => {
						console.log('External diff tool has exited (' + args[args.length - 1] + ')');
						if (errorInfo !== null) {
	...
	private getBranches(repo: string, showRemoteBranches: boolean, hideRemotes: ReadonlyArray<string>) {
		...
		const showRemoteHeads = DataSourceConfig.showRemoteHeads;
	...
	private getUncommittedChanges(repo: string) {
		return this.spawnGit(['status', '--untracked-files=' + (DataSourceConfig.showUntrackedFiles ? 'all' : 'no'), '--porcelain'], repo, (stdout) => {
	...
	private getStatus(repo: string) {
		return this.spawnGit(['status', '-s', '--untracked-files=' + (DataSourceConfig.showUntrackedFiles ? 'all' : 'no'), '--porcelain', '-z'], repo, (stdout) => {
	 ...
	 */
	public execDiff(repo: string, fromHash: string, toHash: string, arg: '--numstat' | '--name-status', filter: string) {
		let args: string[];
		if (fromHash === toHash) {
	...
	/**
	 * Get the diff logs from commit hash.
	 * @param repo The path of the repository.
	 * @param hash The diff logs is from.
	 * @param filePath The special file changes to filter.
	 * @returns The diff string output.
	 */
	public hashDiff(repo: string, hash: string, filePath?: string) {
		let args = ['show', hash,]
		if (filePath) {
			args.push(filePath)
		}
